      ...
      <Modal visible={createOpen} transparent animationType="fade" onRequestClose={() => setCreateOpen(false)}>
        <View style={s.modalBackdrop}>
          <View style={s.modalCard}>
            ...
            {form.password.length > 0 ? (() => { const v = validatePasswordSync(form.password, { email: form.email, nombre: form.fullName, rol: form.rol }); return <Text style={[s.helpText, v.ok ? { color: '#15803D' } : { color: theme.colors.danger }]}>{v.ok ? '✓ Contraseña válida' : v.reasons.join(' · ')}</Text>; })() : <Text style={s.helpText}>Mín. 8–64, sin datos personales, evita secuencias/común. Ej: K7#pL9!q2Xy</Text>}
            <View style={s.rowGap}>
              {ROLES.map((r) => (
