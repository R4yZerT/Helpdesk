...
function getErrorMessage(e: unknown): string {
  ...
  if (e && typeof e === 'object') {
    ...
    try { const j = JSON.stringify(o); if (j !== '{}') return j; } catch { /* sin JSON útil */ }
  }
...
export function AdminMesaTicketsScreen() {
  ...
  const fetchTickets = useCallback(async () => {
    if (mesaId == null) { setTickets([]); setLoading(false); return; }
    ...
    try {
      ...
      const res = await listMyTickets(supabase as never, { mesaId: mesaId as number, q: qDeb || undefined, tecnicoId: tecnicoIdParam, prioridad: (prioridadFilter as never) || undefined, page: 0, pageSize: 50 });
      setTickets(res.data);
    } catch (e) { setErrorMsg(getErrorMessage(e)); } finally { setLoading(false); }
...
const s = StyleSheet.create({
  ...
  header: { paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12, gap: 8 },
